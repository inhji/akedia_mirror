defmodule WebmentionsTest do
  ...
  test "ignores an empty link in header" do
    ...
    with_mock HTTPoison, get: fn _url, _headers, _opts -> {:ok, doc} end do
      assert Akedia.Webmentions.discover_endpoint("http://example.org") == {:ok, nil}
    end
  end
  ...
  test "correctly sends a mention to example.org/webmentions" do
    ...
    with_mock HTTPoison,
      ...
      assert Akedia.Webmentions.send_webmentions("http://example.org") ==
  ...
  test "doesn't send a webmention to a rel=nofollow" do
    ...
    with_mock HTTPoison,
      get: fn _url, _headers, _opts -> {:ok, doc} end,
      post: fn _url, _headers, _opts -> {:ok, doc} end do
      assert Akedia.Webmentions.send_webmentions("http://example.org") == {:ok, []}
    end
  end
  ...
  test "successfully mentions a HTTP ressource" do
    assert Akedia.Webmentions.send_webmentions_for_doc(
             "<html class=\"h-entry\"><a href=\"https://git.inhji.de/repo-avatars/13-1cc19cd6c4cdc1cfaab6e83bd2fb93cd\">blah</a>",
             "http://example.org/"
           ) ==
             {:ok,
              [
