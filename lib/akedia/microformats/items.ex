defmodule Akedia.Microformats2.Items do
  import Akedia.Microformats2.Helpers

  alias Akedia.Microformats2.Items.ImpliedProperties

  def parse(nodes, doc, url, items \\ [])

  def parse([head | tail], doc, url, items) when is_bitstring(head),
    do: parse(tail, doc, url, items)

  def parse([head | tail], doc, url, items),
    do: parse(tail, doc, url, parse(head, doc, url, items))
  ...
  defp parse_sub([child | children], doc, url, item) when is_bitstring(child),
    do: parse_sub(children, doc, url, item)
