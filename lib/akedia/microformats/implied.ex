defmodule Akedia.Microformats2.Items.ImpliedProperties do
  import Akedia.Microformats2.Helpers

  alias Akedia.Microformats2.Items
  ...
  defp implied_name_attrval(node = {"img", _, _}),
    do: Floki.attribute(node, "alt") |> List.first()

  defp implied_name_attrval(node = {"area", _, _}),
    do: Floki.attribute(node, "alt") |> List.first()

  defp implied_name_attrval(node = {"abbr", _, _}),
    do: Floki.attribute(node, "title") |> List.first()
  ...
  defp implied_photo_attrval(node = {"img", _, _}),
    do: Floki.attribute(node, "src") |> List.first()

  defp implied_photo_attrval(node = {"object", _, _}),
    do: Floki.attribute(node, "data") |> List.first()
  ...
  defp implied_url_attrval(node = {"area", _, _}),
    do: Floki.attribute(node, "href") |> List.first()
