defmodule Akedia.Atomex.Entry do
  @moduledoc """
  A feed entry
  """
  ...
  import XmlBuilder
  alias Akedia.Atomex.Types.{Person, Link, Text, Content, Category}
  ...
  See `Akedia.Atomex.Types.Person` for accepted attributes
  """
  def author(entry, name, attributes \\ []),
    do: add_field(entry, Person.new(:author, name, attributes))

  @doc """
  Identifies a related Web page. The type of relation is defined by the rel
  attribute. An entry is limited to one alternate per type and hreflang.
  An entry must contain an alternate link if there is no content element.

  See `Akedia.Atomex.Types.Link` for accepted attributes
  """
  def link(entry, href, attributes \\ []),
    do: add_field(entry, Link.new(href, attributes))

  @doc """
  Contains or links to the complete content of the entry. Content must be
  provided if there is no alternate link, and should be provided if
  there is no summary.

  See `Akedia.Atomex.Types.Content` for accepted attributes
  """
  def content(entry, content \\ "", attributes \\ []),
    do: add_field(entry, Content.new(content, attributes))

  @doc """
  Conveys a short summary, abstract, or excerpt of the entry. Summary
  should be provided if there either is no content provided for the entry,
  or that content is not inline (i.e., contains a src attribute),
  or if the content is encoded in base64.

  See `Akedia.Atomex.Types.Text` for accepted types
  """
  def summary(entry, summary, type \\ "text"),
    do: add_field(entry, Text.new(:summary, summary, type))
  ...
  """
  def published(entry, datetime) do
    add_field(entry, element(:published, nil, DateTime.to_iso8601(datetime)))
  end
  ...
  See `Akedia.Atomex.Types.Person` for accepted attributes
  """
  def contributor(entry, name, attributes \\ []),
    do: add_field(entry, Person.new(:contributor, name, attributes))
  ...
  See `Akedia.Atomex.Types.Text` for accepted types
  """
  def rights(entry, content, type \\ "text"),
    do: add_field(entry, Text.new(:rights, content, type))

  @doc """
  Specifies a category that the entry belongs to.
  A entry may have multiple category elements.

  See `Akedia.Atomex.Types.Category` for accepted attributes
  """
  def category(entry, term, attributes \\ []),
    do: add_field(entry, Category.new(term, attributes))
end
